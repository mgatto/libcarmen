...
/**
 * A city in the world: its identity strings, geographic location, an array
 * of investigable sites, and its connections to other cities.  Plain data
 * with no container bookkeeping (see "Key Design Decisions" in the README).
 */
typedef struct {
...
/** Initialise a city from its id, display names, and geographic location. */
CARMEN_API void              carmen_city_init(CarmenCity *c, const char *id,
                                              ...
                                              double latitude,
                                              double longitude);
/** Append a copied site to the city (bounded by CARMEN_MAX_SITES). */
CARMEN_API void              carmen_city_add_site(CarmenCity *c,
                                                  const CarmenSite *s);
/** Append a copied connection to the city (bounded by CARMEN_MAX_CONNECTIONS). */
CARMEN_API void              carmen_city_add_connection(
                                  CarmenCity *c,
                                  const CarmenConnection *conn);

/**
 * Return 1 if the city has a direct connection to dest_id, else 0. Returns 0
 * for a NULL c or dest_id.
 */
...
/**
 * Write up to max_out borrowed CarmenSite pointers for sites of the given
 * type into out; returns the count written.  Returns 0 if c/type/out is NULL
 * or max_out <= 0.
 */
