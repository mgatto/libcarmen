...
/** One bucket in a secondary index: a grouping key plus the slots it indexes. */
typedef struct {
...
/**
 * The static world: a fixed-capacity array of CarmenCity structs plus a
 * string hash map (city id -> slot) and secondary continent/country indices.
 * Large (~1.4 MB); prefer carmen_world_create()/carmen_world_free() on the
 * heap over stack-declaring it.
 */
typedef struct {
...
/** Initialise a stack-allocated CarmenWorld to an empty world. */
CARMEN_API void         carmen_world_init(CarmenWorld *w);
/** Release a stack-allocated CarmenWorld's internal hash map. */
...
/** Free a world returned by carmen_world_create(). */
 ...
 * continent into out[]; returns the count written.  Each pointer is
...
CARMEN_API int   carmen_world_cities_in_continent(CarmenWorld *w,
                                                   ...
                                                   int max_out);
/**
 * As carmen_world_cities_in_continent(), filtered by country instead of
 * continent.
 */
...
/** Return the number of distinct continents among w's cities (0 for NULL w). */
