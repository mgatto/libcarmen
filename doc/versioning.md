...
Nothing is pushed; pushing the tag triggers `release.yml` to build and publish the demo packages. Push the new tag by name — `git push origin HEAD` then `git push origin v<version>` — never `git push --tags` or `--follow-tags`, which would also push any locally rewritten older tag and trip a protected/immutable remote tag. Useful flags: `--dry-run` (preview), `--release-as minor` (force a bump type), `--first-release` (tag without bumping), `--prerelease <name>`.
