...
{
    for (int i = 0; i < len; i++)
        if (strcmp(trail[i], id) == 0) return 1;
    return 0;
}
...
static const CarmenClue *pick_clue_to(const CarmenSite *site, const char *target)
{
    for (int c = 0; c < site->clue_count; c++)
        if (site->clues[c].type == CARMEN_CLUE_POSITIVE
            && strcmp(site->clues[c].target_city_id, target) == 0)
...
static const CarmenClue *pick_herring_from(const CarmenSite *site, const char *target)
{
    for (int c = 0; c < site->clue_count; c++)
        if (site->clues[c].type == CARMEN_CLUE_POSITIVE
            && strcmp(site->clues[c].target_city_id, target) != 0)
...
static int build_trail(CarmenWorld *w, const char *trail[])
{
    ...
    for (int i = 1; i < TRAIL_LEN; i++) {
        ...
        int n = carmen_world_destinations_from(w, trail[i - 1],
                                                nbr, CARMEN_MAX_CONNECTIONS);
...
int main(void)
{
    ...
    printf("  A suspect has been sighted! Follow the trail.\n");
    printf("%s\n\n", LINE);
    ...
    for (int i = 0; i < TRAIL_LEN; i++) {
        ...
        int ncorrect = find_sites_for(city, next_id, correct_sites, CARMEN_MAX_SITES);
        ...
        typedef struct {
            const CarmenClue *clue; const CarmenSite *site; const char *tag;
        } Slot;
