...
- **Public API surface**: every function callers are meant to use is declared in the matching `include/carmen/<module>.h`, marked `CARMEN_API`, and documented with a short Doxygen `/** ... */` comment above the declaration describing behavior, ownership of any returned pointer, and the meaning of any non-obvious return value (especially negative error codes). These `/** ... */` blocks (not plain `/* ... */`) are what feed the Doxygen API reference built by `make docs` / `.github/workflows/docs.yml`; ordinary internal and inline comments stay `/* ... */`. Keep the umbrella header `include/carmen/carmen.h` and both `Makefile` (`PUBLIC_HEADERS`, `LIB_SRCS`) and `CMakeLists.txt` in sync when adding a new module.
...
6. Update `README.md` / `doc/tier*.md` when you change the public API surface, project layout, or feature status they describe; keep new/changed public API doc comments in Doxygen `/** ... */` form and regenerate the reference with `make docs`.
7. New API example programs live in `examples/` and must be wired into both build systems (Makefile `EXAMPLES` + `all`, and CMakeLists.txt under `CARMEN_BUILD_EXAMPLES`) so CI compiles them.

## Versioning

The top-level `VERSION` file is the single source of truth for the version; `carmen_version.h` is generated from it (see `doc/versioning.md`). Releases are cut with `scripts/release.sh` (a wrapper around `commit-and-tag-version`, configured by `.versionrc.json`), which derives the semver bump from the conventional commits since the last tag, writes `VERSION`, regenerates `CHANGELOG.md`, commits, and tags `vX.Y.Z` -- never hand-edit the version in `Makefile`/`CMakeLists.txt`/`carmen_version.h`. Follow `doc/versioning.md` for what a bump means (major = ABI break), and run `make verify-soname` plus `make version-check` when changing versioning so Make and CMake stay in lockstep (`doc/build-parity.md`).
